...
export default function Header() {
  ...
  const activeLink =
    "text-zinc-900 font-medium"; // era semibold; agora medium
        ...
        <nav className="hidden items-center gap-9 md:flex">
          ...
          <div className="group relative">
            <button className={`${baseLink} inline-flex items-center gap-1`}>
              Produtos
              {/* chevron */}
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="mt-[1px]">
      ...
      {open && (
        <div className="border-t border-zinc-200 bg-white md:hidden">
          <nav className="mx-auto grid max-w-7xl gap-1 px-4 py-3">
            <Link href="/" onClick={() => setOpen(false)} className="rounded-md px-2 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
              Home
            </Link>
            <span className="mt-2 text-xs font-medium text-zinc-500">Produtos</span>
            {products.map((p) => (
              <Link key={p.href} href={p.href} onClick={() => setOpen(false)} className="rounded-md px-2 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
                {p.title}
              </Link>
            ))}
            <span className="mt-2 text-xs font-medium text-zinc-500">Soluções</span>
            {solutions.map((s) => (
              <Link key={s.href} href={s.href} onClick={() => setOpen(false)} className="rounded-md px-2 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
                {s.title}
              </Link>
            ))}
            <Link href="/about-us" onClick={() => setOpen(false)} className="rounded-md px-2 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
              Sobre nós
            </Link>
            <Link href="/impact-investing" onClick={() => setOpen(false)} className="rounded-md px-2 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
              Impacto
            </Link>
